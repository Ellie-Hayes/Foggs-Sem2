...
MeshObject::MeshObject(Mesh* mesh, Texture2D* texture, float x, float y, float z, std::string name) : SceneObject(mesh)
{
	_texture = texture;
	_rotation = rand() % 360;
...
void MeshObject::SetRotation(float rotation)
{
	_rotation = rotation;
}